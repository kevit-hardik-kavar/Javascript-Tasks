...
const getUsers = (users, index = 0) => {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    <td class='tdata'><img id='imgLink' src='${users[i].avatar}' width='40px' height='40px' /></td>
    <td class='tdata'>${users[i].first_name} </td>
    <td class='tdata'>${users[i].last_name}</td>
    <td class='tdata'>${users[i].email}</td>
    <td class='tdata'><i class="fa fa-trash fa-lg" onclick="handleOnDelete(${index})"></i>
    <i class="fa fa-pencil-square-o fa-lg" onclick="handleOnEdit(${index})"></i> 
    </td></tr>`;
...
const handleOnSubmit = () => {
  ...
  // console.log(mail);
  if (firstName == "" && lastName == "" && mail == "" && img == "" ) {
    document.getElementsByTagName("input").style.borderColor = "red";
    document.getElementById("subtn").getAttribute("disabled") = true
...
const handleOnDelete = (index) => {
  ...
  if (confirmDelete === true) {
    const a = document.getElementById(index).remove();
    console.log(a);
  }
};
